...
for (m in 1:chains){
  for (t in 1:(iter-1)){
    ...
    r=dnorm(theta1,0,1)*dnorm(theta2,0,1)*dnorm(sims[t,m,1],theta1,0.2^2)*dnorm(sims[t,m,2],theta2,0.2^2)/(dnorm(sims[t,m,1],0,1)*dnorm(sims[t,m,2],0,1)*dnorm(theta1,sims[t,m,1],0.2^2)*dnorm(theta2,sims[t,m,2],0.2^2))
    #print(r)
    r=min(r,1)
    u=runif(1,0,1)
    if (u<r){
